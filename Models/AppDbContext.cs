...
public class AppDbContext : DbContext, IMiddleware
{
    ...
    public virtual DbSet<ProductActivity> ProductActivities { get; set; }
    ...
    public static void Configure<TAppDbContext>(IServiceCollection services, IConfiguration configuration)
        where TAppDbContext: AppDbContext
    {
        services.AddDbContext<TAppDbContext>(options =>
            
            options.UseInMemoryDatabase(typeof(TAppDbContext).Name)
    ...
    public void BeforeSaveChanges()
    {
        ...
        {
            if(entry.Entity.ToString() == typeof(ProductInfo).FullName)
            {
                ...
                if(save)
                    this.ProductActivities.Add(activity);
            }                                
        }
    }
    ...
    public void Log()
    {
        Console.WriteLine("ProductInfos: "+ this.ProductInfos.Count());
        Console.WriteLine("ProductActivities: "+ this.ProductActivities.Count());
        Console.WriteLine("ProductCatalogs: "+ this.ProductCatalogs.Count());
    ...
    public void Clear()
    {
        foreach (var act in this.ProductActivities.ToList())
            this.Remove(act);
